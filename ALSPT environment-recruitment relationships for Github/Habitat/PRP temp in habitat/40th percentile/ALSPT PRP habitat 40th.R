...
setwd("C:/Users/manue/OneDrive/Desktop/ALSPT recdevs vs environmental covariates/Habitat/PRP temp in habitat/40th percentile")
library(ggplot2)

##################### Delta first
### first temp in area w slainity greater or equal to 1psu
deltat1div <- quantile(deltajunoctmax$T1, probs = 0.40) # division
...
aa <- ggplot(data=deltajunoctmax, aes(x=T1,y=RecDev)) +
  ...
  labs(title="Delta area with salinity ??? 1 PSU", x="Delta max temperature in area with salinity ??? 1 PSU", y="Log recruitment deviation") +
  theme_bw() +
  annotate("text", x=30, y=0.8, label= "Median = -0.12
  SD = 0.44
  Good = 10%
  Poor = 90%",
           col="black", size=3) +
...
#ggsave("PRP_Delta_T1.png", width = 10, height = 8, dpi = 1000)
...
deltat2div <- quantile(deltajunoctmax$T2, probs = 0.40) # division
...
ba <- ggplot(data=deltajunoctmax, aes(x=T2,y=RecDev)) +
  ...
  geom_line(data = data.frame(x = c(deltat2div, Inf), y = c(deltat2notextrec,deltat2notextrec)), aes(x = x , y = y,),
            col="steelblue", size=1) +
  labs(title="Delta area with salinity ??? 2 PSU", x="Delta max temperature in area with salinity ??? 2 PSU", y="Log recruitment deviation") +
  theme_bw() +
  annotate("text", x=30, y=0.8, label= "Median = -0.05
  SD = 0.38
  Good = 0%
  Poor = 100%",
           col="black", size=3) +
  annotate("text", x=33, y=-0.5, label= "Median = 0.64
  SD = 0.48",
           col="black", size=3)
ba

#ggsave("PRP_Delta_T2.png", width = 10, height = 8, dpi = 1000)
...
deltat3div <- quantile(deltajunoctmax$T3, probs = 0.40) # division
...
ca <- ggplot(data=deltajunoctmax, aes(x=T3,y=RecDev)) +
  ...
  geom_line(data = data.frame(x = c(deltat3div, Inf), y = c(deltat3notextrec,deltat3notextrec)), aes(x = x , y = y,),
            col="steelblue", size=1) +
  labs(title="Delta area with salinity ??? 3 PSU", x="Delta max temperature in area with salinity ??? 3 PSU", y="Log recruitment deviation") +
  theme_bw() +
  annotate("text", x=30, y=0.8, label= "Median = -0.05
  SD = 0.44
  Good = 10%
...
#ggsave("PRP_Delta_T3.png", width = 10, height = 8, dpi = 1000)
...
### first temp in area w slainity greater or equal to 1psu
mbt1div <- quantile(mbjunoctmax$T1, probs = 0.40) # division
...
ab <- ggplot(data=mbjunoctmax, aes(x=T1,y=RecDev)) +
  ...
  theme_bw() +
  annotate("text", x=31.25, y=0.8, label= "Median = 0.14
  SD = 0.51
  Good = 40%
  Poor = 60%",
           col="black", size=3) +
...
##########################################################
### second, temp in area w slainity greater or equal to 2psu
mbt2div <- quantile(mbjunoctmax$T2, probs = 0.40) # division
...
##########################################################
### third, temp in area w slainity greater or equal to 3psu
mbt3div <- quantile(mbjunoctmax$T3, probs = 0.40) # division
...
cb <- ggplot(data=mbjunoctmax, aes(x=T3,y=RecDev)) +
  ...
  theme_bw() +
  annotate("text", x=31.25, y=0.8, label= "Median = 0.37
  SD = 0.55
  Good = 60%
  Poor = 40%",
           col="black", size=3) +
  annotate("text", x=33, y=-0.5, label= "Median = 0.25
  SD = 0.49",
           col="black", size=3)
cb
