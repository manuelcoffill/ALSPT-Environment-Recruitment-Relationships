...
setwd(here("Habitat","PRP temp in habitat/30th percentile"))
library(ggplot2)

##################### Delta first
### first temp in area w slainity greater or equal to 1psu
deltat1div <- quantile(deltajunoctmax$T1, probs = 0.30) # division
...
aa <- ggplot(data=deltajunoctmax, aes(x=T1,y=RecDev)) +
  ...
  theme_bw() +
  annotate("text", x=30, y=0.8, label= "Median = -0.17
  SD = 0.39
  Good = 14%
  Poor = 86%",
           col="black", size=3) +
  annotate("text", x=33, y=-0.5, label= "Median = 0.47
  SD = 0.42",
           col="black", size=3)
aa
...
deltat2div <- quantile(deltajunoctmax$T2, probs = 0.30) # division
...
ba <- ggplot(data=deltajunoctmax, aes(x=T2,y=RecDev)) +
  ...
  Poor = 86%",
           col="black", size=3) +
  annotate("text", x=33, y=-0.5, label= "Median = 0.46
  SD = 0.46",
           col="black", size=3)
ba
...
deltat3div <- quantile(deltajunoctmax$T3, probs = 0.30) # division
...
ca <- ggplot(data=deltajunoctmax, aes(x=T3,y=RecDev)) +
  ...
  Poor = 100%",
           col="black", size=3) +
  annotate("text", x=33, y=-0.5, label= "Median = 0.48
  SD = 0.4",
           col="black", size=3)
ca
...
### first temp in area w slainity greater or equal to 1psu
mbt1div <- quantile(mbjunoctmax$T1, probs = 0.30) # division
...
ab <- ggplot(data=mbjunoctmax, aes(x=T1,y=RecDev)) +
  ...
  Poor = 71%",
           col="black", size=3) +
  annotate("text", x=33, y=-0.5, label= "Median = 0.32
  SD = 0.47",
           col="black", size=3)
ab
...
##########################################################
### second, temp in area w slainity greater or equal to 2psu
mbt2div <- quantile(mbjunoctmax$T2, probs = 0.30) # division
...
##########################################################
### third, temp in area w slainity greater or equal to 3psu
mbt3div <- quantile(mbjunoctmax$T3, probs = 0.30) # division
...
#ggsave("PRP_Merged30th.png", width = 8, height = 10, dpi = 1000)
