...
setwd(here("Habitat","PRP temp in habitat","20th percentile"))
library(ggplot2)

##################### Delta first
### first temp in area w slainity greater or equal to 1psu
deltat1div <- quantile(deltajunoctmax$T1, probs = 0.2) # division
...
aa <- ggplot(data=deltajunoctmax, aes(x=T1,y=RecDev)) +
  ...
  theme_bw() +
  annotate("text", x=30, y=0.8, label= "Median = -0.20
  SD = 0.31
  Good = 0%
  Poor = 100%",
           col="black", size=3) +
...
deltat2div <- quantile(deltajunoctmax$T2, probs = 0.2) # division
...
deltat3div <- quantile(deltajunoctmax$T3, probs = 0.2) # division
...
### first temp in area w slainity greater or equal to 1psu
mbt1div <- quantile(mbjunoctmax$T1, probs = 0.2) # division
...
##########################################################
### second, temp in area w slainity greater or equal to 2psu
mbt2div <- quantile(mbjunoctmax$T2, probs = 0.2) # division
...
##########################################################
### third, temp in area w slainity greater or equal to 3psu
mbt3div <- quantile(mbjunoctmax$T3, probs = 0.2) # division
...
#ggsave("PRP_Merged20th.png", width = 8, height = 10, dpi = 1000)
