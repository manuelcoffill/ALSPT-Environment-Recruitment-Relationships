...
e <- ggplot(data = nao, aes(Year, Annual)) +
  geom_bar(stat = "identity", col="black", fill="#0d0887") +
  geom_hline(yintercept=0, col="black", lty=1, size=1) +
...
f <- ggplot(data = naoforrec, aes(Year, Annual)) +
  geom_bar(stat = "identity", col="black", fill="#0d0887") +
  geom_hline(yintercept=0, col="black", lty=1, size=1) +
  scale_y_continuous(breaks = c(-1.2,-0.8,-0.4,0,0.4,0.8,1.2)) +
...
g <- ggplot(data = nao, aes(Year, `Jun-Oct`)) +
  geom_bar(stat = "identity", col="black", fill="#0d0887") +
  geom_hline(yintercept=0, col="black", lty=1, size=1) +
...
h <- ggplot(data = naoforrec, aes(Year, `Jun-Oct`)) +
  geom_bar(stat = "identity", col="black", fill="#0d0887") +
  geom_hline(yintercept=0, col="black", lty=1, size=1) +
  scale_y_continuous(breaks = c(-1.5,-1,-0.5,0,0.5,1,1.5)) +
...
finall <- ggarrange(e,g,f,h, ncol = 2, nrow = 2
                   #label.x = c(0.85, 0.85),  # the x positions of the labels
                   #label.y = c(1, 1),
                   #font.label = list(size = 30)  # t)
)
...
################################################################################
###merging period both for period used
mergedfinal <- ggarrange(b,f,d,h, ncol = 2, nrow = 2
                    #label.x = c(0.85, 0.85),  # the x positions of the labels
                    #label.y = c(1, 1),
                    #font.label = list(size = 30)  # t)
)
...
mergedfinal
annotate_figure(mergedfinal, bottom = textGrob("Year", vjust = 0),
                left = textGrob("Index", vjust = 0, rot=90))

#ggsave("AMONAOMergedIndices.png", width = 10, height = 8, dpi = 1000)
